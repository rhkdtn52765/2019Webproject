...
<div class='container' style='margin-top:100px;min-height:500px'>
	<h2>"${search_product}"에 대한 검색 결과</h2>
	<div class='row'>
		<c:forEach var="obj" items = "${requestScope.search_result}" >
			<div class='col-sm-3'>
...
<div class='container' style='margin-top:50px'>
   ...
   <a href="search_product.springshoppingmall?page_num=${page_bean.pagination_min - 1}&search_product=${search_product}"class ='page-link'>이전</a>
   </c:if>
   
   <c:forEach var="idx" begin="${page_bean.pagination_min }" end="${page_bean.pagination_max }">
      <a href="search_product.springshoppingmall?page_num=${idx}&search_product=${search_product}" class ='page-link'>${idx }</a>
   </c:forEach>
   
   <c:if test="${page_bean.pagination_max < page_bean.page_cnt}">
   <a href="search_product.springshoppingmall?page_num=${page_bean.pagination_max + 1}&search_product=${search_product}"class ='page-link'>다음</a>
   </c:if>
